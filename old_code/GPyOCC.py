...
def train(train_x, train_y, model, likelihood, num_steps=500):
    ...
    # This does not work for OCC problems
...
# def transform_expert(mod):  # I honestly do not know what this function does here: experts aren't modeled via OCC
...
def run_model(train_mod, test_mod, train_w, train_h, test_w, test_h):
    X_train, y_train, X_train_orig, train_col_names = load_data.load_xy(train_mod, model='hist_buildings')
    X_test, y_test, X_test_orig, test_col_names = load_data.load_xy(test_mod, model='hist_buildings')
